  ...
  module Versions
    ...
    def self.matches?(version : Version, requirement : VersionReq)
      requirement.parsed_patterns.all? do |pattern|
        matches_parsed_pattern?(version, pattern)
      end
    end

    private def self.matches_parsed_pattern?(version : Version, pattern : VersionReq::ParsedPattern)
      case pattern[0]
      when VersionReq::PatternType::Any
        true
      when VersionReq::PatternType::Approximate
        matches_approximate?(version.value, pattern[1], pattern[2])
      else # Operator
        matches_operator?(version.value, pattern[1], pattern[2])
      end
    end

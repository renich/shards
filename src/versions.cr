module Shards
  module Versions
    ...
    private def self.matches_parsed_pattern?(version : Version, pattern : ParsedPattern)
      case pattern.kind
      in PatternKind::Any
        true
      in PatternKind::Approximate
        matches_approximate?(version.value, pattern.op, pattern.req)
      in PatternKind::Operator
        matches_operator?(version.value, pattern.op, pattern.req)
      end
    end

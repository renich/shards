module Shards
  module Versions
    ...
    private def self.matches_single_pattern?(version : Version, pattern : VersionReq::Pattern)
      case pattern.operator
      when VersionReq::Operator::ANY
        true
      when VersionReq::Operator::APPROXIMATE
        matches_approximate?(version.value, pattern.requirement, pattern.ver.not_nil!)
      when VersionReq::Operator::GREATER_OR_EQUAL
        compare(version.value, pattern.requirement) <= 0
      when VersionReq::Operator::LESS_OR_EQUAL
        compare(version.value, pattern.requirement) >= 0
      when VersionReq::Operator::GREATER
        compare(version.value, pattern.requirement) < 0
      when VersionReq::Operator::LESS
        compare(version.value, pattern.requirement) > 0
      when VersionReq::Operator::NOT_EQUAL
        compare(version.value, pattern.requirement) != 0
      else # EQUAL
        compare(version.value, pattern.requirement) == 0
      end
